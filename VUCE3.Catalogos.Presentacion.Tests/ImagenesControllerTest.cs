...
namespace VUCE3.Catalogos.Presentacion.Tests
{
    public class ImagenesControllerTest
    {
        ...
        {
            //Arange
            var lstImagenes = new List<Imagenes>()
            {
                new Imagenes()
                {
                    Id =1,
            ...
            mockSender.Setup(s => s.Send(It.IsAny<ObtenerImagenesQuery>(), It.IsAny<CancellationToken>())).ReturnsAsync(lstImagenes);
            
            var controller = new ImagenesController(mockSender.Object, AutoMapperSingleton.Mapper, mockLocalizer.Object);
            var result = await controller.Get();
            var queryResult = Assert.IsType<OkObjectResult>(result);
            var querydto = Assert.IsType<List<ImagenesDto>>(queryResult.Value);

            //Assert
            Assert.IsType<OkObjectResult>(result);
            Assert.IsType<List<ImagenesDto>>(queryResult.Value);
            Assert.Equal(1, querydto[0].Id);
            Assert.Equal(dataImageMock64(), querydto[0].Imagen);            
            
        }

        [Fact]
        public async Task ObtenerImagenes_Error()
        {
            // Error
            var error = ErroresImagenes.NoEncontrada;
            var resourceLang = new LocalizedString("Imagen.NoEncontrada", "Image not found");

            //Act
            var mockRepo = new Mock<IUnitOfWork>();
            var mockSender = new Mock<ISender>();
            mockSender.Setup(s => s.Send(It.IsAny<ObtenerImagenesQuery>(), It.IsAny<CancellationToken>())).ReturnsAsync(error);
            mockLocalizer.Setup(r => r["Imagen.NoEncontrada"]).Returns(resourceLang);
        ...
        public async Task ObtenerImagenPorId_Ok()
        {
            ...
            var mockRepo = new Mock<IUnitOfWork>();
            var mockSender = new Mock<ISender>();
            mockSender.Setup(s => s.Send(It.IsAny<ObtenerImagenesPorIdQuery>(), It.IsAny<CancellationToken>())).ReturnsAsync(imagen);

            var controller = new ImagenesController(mockSender.Object, AutoMapperSingleton.Mapper, mockLocalizer.Object);
            var result = await controller.Get(1);
            var queryResult = Assert.IsType<OkObjectResult>(result);
            var querydto = Assert.IsType<ImagenesDto>(queryResult.Value);

            //Assert
            Assert.IsType<OkObjectResult>(result);
            Assert.IsType<ImagenesDto>(queryResult.Value);
            Assert.Equal(1, querydto.Id);
            Assert.Equal(dataImageMock64(), querydto.Imagen);
        }

        [Fact]
        public async Task ObtenerImagenPorId_Error()
        {
            var error = ErroresImagenes.NoEncontrada;
            var resourceLang = new LocalizedString("Imagen.NoEncontrada", "Image not found");
            ...
            var mockRepo = new Mock<IUnitOfWork>();
            var mockSender = new Mock<ISender>();
            mockSender.Setup(s => s.Send(It.IsAny<ObtenerImagenesPorIdQuery>(), It.IsAny<CancellationToken>())).ReturnsAsync(error);
            mockLocalizer.Setup(r => r["Imagen.NoEncontrada"]).Returns(resourceLang);
            ...
            var errorRes = Assert.IsType<ODataErrorResult>(result);
            Assert.Equal("404", errorRes.Error.ErrorCode);
            Assert.Equal("Image not found", errorRes.Error.Message);
        }

        [Fact]
        public async Task ObtenerImagenPorId_InvalidModel()
        {
            var mockSender = new Mock<ISender>();
        ...
        [Fact]
        public async Task PostImagen_DevuelveCreated()
        {
            var imagenDto = new ImagenesDto()
            {
                Id = 1,
                Imagen = dataImageMock64()
            };
            ...
            mockMediator.Setup(m => m.Send(It.IsAny<CrearImagenesCommand>(), It.IsAny<CancellationToken>())).ReturnsAsync(imagen);

            var controller = new ImagenesController(mockMediator.Object, AutoMapperSingleton.Mapper, mockLocalizer.Object);

            var result = await controller.Post(imagenDto);

            var okResult = Assert.IsType<CreatedODataResult<ImagenesDto>>(result);
            var resultImagen = Assert.IsType<ImagenesDto>(okResult.Value);
            Assert.Equal(dataImageMock64(), resultImagen.Imagen);            
        }

        [Fact]
        public async Task PostImagen_DevuelveBadRequest()
        {
            var imagenDto = new ImagenesDto()
            {
                Id = 1,
        ...
        [Fact]
        public async Task PostImagen_DevuelveProblem()
        {
            var resourceLang = new LocalizedString("General.Failure", "A failure has occurred");
        ...
        [Fact]
        public async Task PatchImagen_InvalidModel()
        {            
            ...
            var result = await controller.Patch(1, new Delta<ImagenesDto>());
            Assert.IsType<ODataErrorResult>(result);
        }

        [Fact]
        public async Task PatchImagen_Ok()
        {            
            var imagen = new Imagenes
            {
                Id = 32,
            ...
            var delta = new Delta<ImagenesDto>(imagenDto.GetType());
            delta.TrySetPropertyValue(nameof(imagenDto.Imagen), dataImageMock64());

            var mockRepo = new Mock<IUnitOfWork>();

            var mockSender = new Mock<ISender>();
            mockSender.Setup(s => s.Send(It.IsAny<EditarImagenesCommand>(), It.IsAny<CancellationToken>()))
                .ReturnsAsync(Tuple.Create(imagen, imagen));
        ...
        [Fact]
        public async Task PatchImagen_NotFound()
        {
            var error = ErroresImagenes.NoEncontrada;
            var resourceLang = new LocalizedString("Imagen.NoEncontrada", "Image not found");
            ...
            var mockSender = new Mock<ISender>();
            mockSender.Setup(s => s.Send(It.IsAny<EditarImagenesCommand>(), It.IsAny<CancellationToken>())).ReturnsAsync(error);
            mockLocalizer.Setup(r => r["Imagen.NoEncontrada"]).Returns(resourceLang);
        ...
        [Fact]
        public async Task DeleteImagen_NotFound()
        {
            var error = ErroresImagenes.NoEncontrada;
            var resourceLang = new LocalizedString("Imagen.NoEncontrada", "Image not found");
            ...
            var mockSender = new Mock<ISender>();
            mockSender.Setup(s => s.Send(It.IsAny<EliminarImagenesCommand>(), It.IsAny<CancellationToken>())).ReturnsAsync(error);
            mockLocalizer.Setup(r => r["Imagen.NoEncontrada"]).Returns(resourceLang);
        ...
        [Fact]
        public async Task DeleteImagen_Ok()
        {
            var imagen = new Imagenes
            {
                Id = 32,
            ...
            var mockSender = new Mock<ISender>();
            mockSender.Setup(s => s.Send(It.IsAny<EliminarImagenesCommand>(), It.IsAny<CancellationToken>())).ReturnsAsync(imagen);
        ...
        [Fact]
        public async Task DeleteImagen_InvalidModel()
        {
            var mockSender = new Mock<ISender>();
        ...
        [Fact]
        public async Task EliminarMasivamenteImagenes_DevuelveOk()
        {
            var param = new ODataActionParameters();
            param.Add("items", new List<int> { 1, 2, 3 });

            mockMediator.Setup(m => m.Send(It.IsAny<EliminarMasivoImagenesCommand>(), It.IsAny<CancellationToken>())).ReturnsAsync(Result.Deleted);
        ...
        [Fact]
        public async Task EliminarMasivamenteImagenes_DevuelveError()
        {
            var param = new ODataActionParameters();
            param.Add("items", new List<int> { 1, 2, 3 });
            ...
            mockMediator.Setup(m => m.Send(It.IsAny<EliminarMasivoImagenesCommand>(), It.IsAny<CancellationToken>())).ReturnsAsync(Error.Failure());
            mockLocalizer.Setup(r => r["General.Failure"]).Returns(resourceLang);
        ...
        [Fact]
        public async Task EliminarMasivamenteImagenes_DevuelveErrorModeloInvalido()
        {
            var controller = new ImagenesController(mockMediator.Object, AutomapperSingleton.Mapper, mockLocalizer.Object);
